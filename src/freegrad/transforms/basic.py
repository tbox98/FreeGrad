...
def rectangular(ctx, grad_out: torch.Tensor, input: torch.Tensor, a: float = -0.5, b: float = 0.5, **_):
    mask = (input >= a) & (input <= b)
    return grad_out * mask.to(grad_out.dtype)

@register("triangular")
def triangular(ctx, grad_out: torch.Tensor, input: torch.Tensor, width: float = 1.0, **_):
...
@register("clip_norm")
def clip_norm(ctx, grad_out: torch.Tensor, input, max_norm: float = 1.0, eps: float = 1e-12, **_):
...
@register("centralize")
def centralize(ctx, grad_out: torch.Tensor, input, dim: int = -1, keepdim: bool = True, **_):
